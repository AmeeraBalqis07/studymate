  ...
  <?php while ($row = $result->fetch_assoc()) { ?>
    <div class="discussion-card">
      ...
      <div class="comment-card">
        ...
        <!-- Delete Button (Visible only for the user who posted the comment) -->
       ...
       $reply_sql = "SELECT r.reply_text, r.timestamp, u.username AS replier_name, r.reply_id, r.user_id AS reply_user_id, r.attachment, u.profile_photo
        ...
        while ($reply_row = $reply_result->fetch_assoc()) { ?>
          <div class="reply-card">
            ...
            <!-- Display attachment for replies -->
            <?php if (isset($reply_row['attachment']) && !empty($reply_row['attachment'])) { ?>
              <div class="reply-attachment">
                <?php 
                $file_type = pathinfo($reply_row['attachment'], PATHINFO_EXTENSION);
                if (in_array($file_type, ['jpg', 'jpeg', 'png', 'gif', 'bmp'])) { ?>
                  <img src="<?php echo $reply_row['attachment']; ?>" alt="Attachment" class="attachment-img">
                <?php } elseif ($file_type === 'pdf') { ?>
                  <a href="<?php echo $reply_row['attachment']; ?>" target="_blank" class="attachment-link">Download PDF</a>
                <?php } elseif (in_array($file_type, ['mp4', 'avi', 'mov'])) { ?>
                  <video width="320" height="240" controls>
                    <source src="<?php echo $reply_row['attachment']; ?>" type="video/<?php echo $file_type; ?>">
                    Your browser does not support the video tag.
                  </video>
                <?php } ?>
              </div>
            ...
            <?php if ($user_id && $user_id == $reply_row['reply_user_id']) { ?>
